...
WindData Alliance membership credentials have validity periods. That is intentional. Membership should be renewed, not assumed forever.

If GridSight's membership credential expires, its next attempt to negotiate access to a member-only offer can fail. The provider does not need to maintain a separate calendar entry for GridSight. The policy asks for a valid credential, and the expired credential no longer satisfies it.

When GridSight renews its membership and receives a new credential, future negotiations can succeed again if the other policy conditions are met.

## A credential is revoked

Expiration is predictable. Revocation is immediate.

Suppose WindData Alliance suspends a participant because it no longer meets the dataspace rules. WindData Alliance revokes the participant's membership credential or marks it as no longer valid.
...
GreenSteel may decide that the GS-87 steel mill certificate should no longer be available to every active member. Perhaps the next version contains more sensitive sourcing or carbon-footprint details.

GreenSteel changes the policy from:

> Active WindData Alliance membership required.

To:

> Active WindData Alliance membership and an approved customer-relationship credential required.

The offer is still published, but fewer consumers can access it. NorthSea Wind or TowerWorks may still qualify. GridSight may not, unless it receives the additional credential or uses a different summary offer designed for analytics.
...
If a company leaves WindData Alliance, the dataspace authority can stop issuing or renewing its membership credential. Providers may also remove participant-specific offers, rotate access tokens, or end agreements according to the governance rules.
